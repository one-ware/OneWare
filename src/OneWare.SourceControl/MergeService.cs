...
public class MergeService : IBackgroundRenderer
{
        ...
        textEditor.TextChanged += (_, _) =>
        {
            ...
            foreach (var merge in Merges)
            {
                ...
                stack.Children.Add(
                    ...
                    {
                        Content = "Keep HEAD",
                        Command = new RelayCommand<MergeEntry>(MergeKeepCurrent),
                ...
                stack.Children.Add(
                    new Button
                    {
                        ...
                        Classes = { "MergeButton" },
                        Command = new RelayCommand<MergeEntry>(MergeKeepIncoming),
                        CommandParameter = merge
                    });
                ...
                stack.Children.Add(
                    new Button
                    {
                        Content = "Keep Both",
                        Classes = { "MergeButton" },
                        Command = new RelayCommand<MergeEntry>(MergeKeepBoth),
                        CommandParameter = merge
                    });
    ...
    {
        if (entry != null) Merge(entry, MergeMode.KeepIncoming);
    }

    public void MergeKeepCurrent(MergeEntry? entry)
    {
        if (entry != null) Merge(entry, MergeMode.KeepCurrent);
    }

    public void MergeKeepBoth(MergeEntry? entry)
    {
        if (entry != null) Merge(entry, MergeMode.KeepBoth);
    }
