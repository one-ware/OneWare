...
namespace OneWare.SourceControl.ViewModels
{
    public class SourceControlViewModel : ExtendedTool
    {
        ...
        public SourceControlViewModel(
            ILogger<SourceControlViewModel> logger,
            ...
            GithubLoginProvider githubLoginProvider) : base(IconKey)
        {
            _logger = logger;
            ...
            _settingsService.GetSettingObservable<double>("SourceControl_AutoFetchDelay")
                .Subscribe(SetupFetchTimer);

            _settingsService.GetSettingObservable<double>("SourceControl_PollChangesDelay")
                .Subscribe(SetupPollTimer);

            _projectExplorerService
                .WhenValueChanged(x => x.ActiveProject)
                .Subscribe(_ => RefreshAsyncCommand.Execute(null));
        ...
        private async Task CompareChangesAsync(Repository repository, string path, string titlePrefix, int contextLines = 3, bool switchTab = true)
        {
            await WaitUntilFreeAsync();
            try
            {
                ...
                openTab ??= new CompareFileViewModel(fullPath, this);
        ...
        private async Task<Credentials> GetCredentialsAsync(string url, string usernameFromUrl, SupportedCredentialTypes types, CancellationToken cancellationToken = default)
        {
            if (types.HasFlag(SupportedCredentialTypes.UsernamePassword))
            {
                var ub = new Uri(url);

                var username = _settingsService.GetSettingValue<string>("GitHubAccountNameKey");
