...
namespace OneWare.Verilog
{
    public class LanguageServiceVerilog(string workspace, ISettingsService settingsService, IPackageService packageService)
        : LanguageServiceLspAutoDownload(settingsService.GetSettingObservable<string>(VerilogModule.LspPathSetting), VerilogModule.VeriblePackage,
            VerilogModule.LspName, workspace, packageService, settingsService.GetSettingObservable<bool>("Experimental_AutoDownloadBinaries"))
    {
        ...
        {
            if (file is IProjectFile pf && pf.TopFolder?.SearchName(Path.GetFileNameWithoutExtension(file.FullPath) + ".qip") != null)
