    ...
    public abstract class LanguageServiceBase
    {
        ...
        public async Task DeactivateAsync()
        {
            ...
            await Dispatcher.UIThread.InvokeAsync(async () =>
            {
                ...
                ContainerLocator.Container.Resolve<ErrorListViewModel>()?.Clear(ErrorSource);
        ...
        protected async Task InitAsync(Process process, Action<LanguageClientOptions>? customOptions = null)
        {
            ...
            Client = LanguageClient.PreInit(
                options =>
                {
                    options.WithClientInfo(new ClientInfo { Name = "OneWare.Core", Version = Global.VersionCode });
        ...
        public void ApplyContainer(string path, IEnumerable<TextEdit> con)
        {
            var openDoc =
                ContainerLocator.Container.Resolve<IDockService>().OpenFiles
                    .FirstOrDefault(x => x.Key.FullPath.Equals(path, StringComparison.OrdinalIgnoreCase)).Value as EditViewModel;
        ...
        public void PublishDiag(PublishDiagnosticsParams pdp)
        {
            Dispatcher.UIThread.Post(() =>
            {
                ...
                ContainerLocator.Container.Resolve<ErrorListViewModel>().RefreshErrors(ConvertErrors(pdp, file).ToList(), ErrorSource, file);
