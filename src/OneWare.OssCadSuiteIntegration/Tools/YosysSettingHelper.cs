...
{
    private static readonly IImage? _icon =
        Application.Current!.FindResource(ThemeVariant.Dark, "ForkAwesome.Check") as IImage;
    ...
    public static Task UpdateProjectPcFileAsync(IProjectFile file)
    {
        ...
        foreach (var projectFile in file.Root.Files)
            if (_icon != null)
                projectFile.IconOverlays.Remove(_icon);
    ...
    public static void UpdateProjectProperties(UniversalFpgaProjectRoot project, string? constraintFile)
    {
        ...
        if (ccfInclude) includeArray.Add("*.pcf");
