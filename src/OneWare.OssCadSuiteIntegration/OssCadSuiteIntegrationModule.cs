...
public class OssCadSuiteIntegrationModule : IModule
{
    ...
    public void OnInitialized(IContainerProvider containerProvider)
    {
        ...
        containerProvider.Resolve<IWindowService>().RegisterUiExtension("CompileWindow_TopRightExtension", new UiExtension(x =>
        {
            if (x is not UniversalFpgaProjectCompileViewModel cm) return null;
                return new YosysCompileWindowExtensionView()
                {
                    DataContext = containerProvider.Resolve<YosysCompileWindowExtensionViewModel>((typeof(UniversalFpgaProjectCompileViewModel), cm))
                };
        }));
        containerProvider.Resolve<IWindowService>().RegisterUiExtension("UniversalFpgaToolBar_DownloaderConfigurationExtension", new UiExtension(x =>
        {
            ...
            {
                DataContext = containerProvider.Resolve<OpenFpgaLoaderWindowExtensionViewModel>((typeof(UniversalFpgaProjectRoot), cm))
            };
        }));
        ...
        containerProvider.Resolve<IProjectExplorerService>().RegisterConstructContextMenu((x,l) =>
        {
            ...
            if (x is [IProjectFile {Extension: ".vcd"} vcd] && IsOssPathValid(settingsService.GetSettingValue<string>("OssCadSuite_Path")))
            {
                l.Add(new MenuItemViewModel("GtkWaveOpen")
                {
                    Header = "Open with GTKWave",
                    Command = new RelayCommand(() => containerProvider.Resolve<GtkWaveService>().OpenInGtkWave(vcd.FullPath))
                });
            }
        });
    }
