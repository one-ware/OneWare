...
public static class HardwareGuiCreator
{
    ...
    public static async Task<HardwareGuiViewModel> CreateGuiAsync(string guiPath, IHardwareModel hardwareModel, ILogger logger)
    {
        ...
        try
        {
            ...
            if (gui.TryGetProperty("elements", out var elementsProperty))
            {
                foreach (var element in elementsProperty.EnumerateArray())
                {
                    ...
                    var width = element.TryGetProperty("width", out var widthProperty) ? widthProperty.GetDouble() : 0;
                    var height = element.TryGetProperty("height", out var heightProperty) ? heightProperty.GetDouble() : 0;
                    var rotation = element.TryGetProperty("rotation", out var rotationProperty) ? rotationProperty.GetDouble() : 0;
                    ...
                    var foreground = element.TryGetProperty("textColor", out var textColorProperty)
                        ...
                        : null;
                    var bind = element.TryGetProperty("bind", out var bindProperty) ? bindProperty.GetString() : null;
                    var connectorStyle = element.TryGetProperty("connectorStyle", out var connectorStyleProperty) ? connectorStyleProperty.GetString() : "default";
                    var fontSize = element.TryGetProperty("fontSize", out var fontSizeProperty) ? fontSizeProperty.GetInt32() : 10;
                    ...
                    switch (element.GetProperty("type").GetString()?.ToLower())
                    {
                        case "gui":
                            {
                                ...
                                var child = await CreateGuiAsync(path, hardwareModel, logger);
                        ...
                        case "rect":
                            {
                                var fontWeightStr = element.TryGetProperty("fontWeight", out var fontWeightProperty)
                                    ...
                                    : null;
                                Enum.TryParse<FontWeight>(fontWeightStr ?? "Normal", true, out var fontWeight);
                        ...
                        case "pinarray":
                            {
                                ...
                                var pinWidth = element.TryGetProperty("pinWidth", out var pinWidthProperty) ? pinWidthProperty.GetInt32() : 10;
                                var pinHeight = element.TryGetProperty("pinHeight", out var pinHeightProperty) ? pinHeightProperty.GetInt32() : 10;
                                ...
                                if (labelPosition == null && element.TryGetProperty("flipLabel", out var flipLabelProperty) && flipLabelProperty.GetBoolean())
                                {
                                    labelPositionEnum = PinLabelPosition.After;
                                }

                                var isHorizontal = element.TryGetProperty("horizontal", out var horizontalProperty) && horizontalProperty.GetBoolean();
                                var list = new List<FpgaGuiElementPinViewModel>();

                                foreach (var pin in element.GetProperty("pins").EnumerateArray())
                                {
                                    var bindPin = pin.TryGetProperty("bind", out var bindPinProperty) ? bindPinProperty.GetString() : null;
                                    var labelPin = pin.TryGetProperty("label", out var labelPinProperty) ? labelPinProperty.GetString() : null;
                        ...
                        case "pin":
                            {
                                color ??= Brushes.YellowGreen;
                                var label = element.TryGetProperty("label", out var labelProperty) ? labelProperty.GetString() : null;
                                var labelPosition = element.TryGetProperty("labelPosition", out var labelPositionProperty)
                                    ? labelPositionProperty.GetString()
                                    : null;
                                Enum.TryParse<PinLabelPosition>(labelPosition, true, out var labelPositionEnum);
                        ...
                        case "usb":
                            {
                                var bindRx = element.TryGetProperty("rxBind", out var bindRxProperty) ? bindRxProperty.GetString() : null;
                                var bindTx = element.TryGetProperty("txBind", out var bindTxProperty) ? bindTxProperty.GetString() : null;
