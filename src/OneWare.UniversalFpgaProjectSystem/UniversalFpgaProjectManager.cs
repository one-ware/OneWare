...
namespace OneWare.UniversalFpgaProjectSystem
{
    public class UniversalFpgaProjectManager : IProjectManager
    {
        ...
        {
            var root = await UniversalFpgaProjectParser.DeserializeAsync(path, _logger);
            ...
            if (top != null && root.SearchRelativePath(top.ToPlatformPath()) is { } entity)
            ...
            if (loader != null && _fpgaService.Loaders.FirstOrDefault(x => x.Name == loader) is { } l)
                root.Loader = l;
        ...
        {
            if (root is UniversalFpgaProjectRoot uFpga)
            {
                return await UniversalFpgaProjectParser.SerializeAsync(uFpga, _logger);
            }
            return false;
        }
        ...
        public async Task NewProjectDialogAsync()
        {
            ...
            {
                DataContext = new UniversalFpgaProjectCreatorViewModel(_windowService, _fpgaService)
            });
        }
        ...
        private async Task OpenProjectSettingsDialogAsync(UniversalFpgaProjectRoot root)
        {
            ...
            {
                DataContext = new UniversalFpgaProjectSettingsEditorViewModel(root, _windowService, _fpgaService)
            });
        }
    }
}
