    ...
    public class UniversalFpgaProjectSettingsEditorViewModel : FlexibleWindowViewModelBase
    {
            ...
            ILogger logger)
        {
            ...
            if (root.TopEntity != null && (root.TopEntity.Name.Contains("vhd") || root.TopEntity.Name.Contains("vhdl")))
            {
                ...
                _vhdlStandard = new ComboBoxSetting("VHDL Standard", value, new[] { "87", "93", "93c", "00", "02", "08", "19" });
            }
                    ...
                    switch (localCopy.GetType().Name)
                    {
                        case "CheckBoxSetting":
                            localCopy = new CheckBoxSetting(localCopy.Title,
                                _root.Properties[setting.Key]!.ToString() == "True");
                            break;
                        ...
                        case "SliderSetting":
                            localCopy = new SliderSetting(localCopy.Title,
                                ...
                                ((SliderSetting)localCopy).Max,
                                ((SliderSetting)localCopy).Step);
                            break;

                        case "FolderPathSetting":
                            localCopy = new FolderPathSetting(localCopy.Title,
                                _root.Properties[setting.Key]!.ToString(),
                                ((FolderPathSetting)localCopy).Watermark,
                                ((FolderPathSetting)localCopy).StartDirectory,
                                ((FolderPathSetting)localCopy).CheckPath);
                            break;

                        case "FilePathSetting":
                            localCopy = new FilePathSetting(localCopy.Title,
                                _root.Properties[setting.Key]!.ToString(),
                                ((FilePathSetting)localCopy).Watermark,
                                ((FilePathSetting)localCopy).StartDirectory,
        ...
        private async Task SaveAsync()
        {
            ...
            foreach (var toSave in SettingsCollection.SettingModels)
            {
                ...
                {
                    case "CheckBoxSetting":
                        _root.SetProjectProperty(_dynamicSettingsKeys[(TitledSetting)toSave],
                            toSave.Value.ToString()!);
                        break;

                    case "TextBoxSetting":
                        _root.SetProjectProperty(_dynamicSettingsKeys[(TitledSetting)toSave], toSave.Value.ToString()!);
                        break;
                    ...
                    case "ListBoxSetting":
                        _root.SetProjectPropertyArray(_dynamicSettingsKeys[(TitledSetting)toSave],
