...
namespace OneWare.UniversalFpgaProjectSystem.Services
{
    public class FpgaService
    {
        ...
        public FpgaService(IPaths paths, ILogger logger, Func<Type, object> resolveService)
        {
        ...
        public void RegisterFpgaExtensionPackage(IFpgaExtensionPackage fpgaExtension)
        {
            ...
            if (existing != null)
                FpgaExtensionPackages.Remove(existing);
        ...
        {
            Toolchains.Add((IFpgaToolchain)_resolveService(typeof(T)));
        }

        public void RegisterLoader<T>() where T : IFpgaLoader
        {
            Loaders.Add((IFpgaLoader)_resolveService(typeof(T)));
        }

        public void RegisterSimulator<T>() where T : IFpgaSimulator
        {
            Simulators.Add((IFpgaSimulator)_resolveService(typeof(T)));
        }

        public void RegisterTemplate<T>() where T : IFpgaProjectTemplate
        {
            Templates.Add((IFpgaProjectTemplate)_resolveService(typeof(T)));
        }

        public void RegisterPreCompileStep<T>() where T : IFpgaPreCompileStep
        {
            PreCompileSteps.Add((IFpgaPreCompileStep)_resolveService(typeof(T)));
        }
        ...
        {
            if (NodeProviders.TryGetValue(extension, out var provider))
                return (INodeProvider)_resolveService(provider);
