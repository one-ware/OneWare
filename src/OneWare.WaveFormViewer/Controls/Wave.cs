    ...
    public class Wave : Control
    {
        ...
        protected override void OnDataContextChanged(EventArgs e)
        {
            ...
            if (DataContext is WaveModel model)
            {
                ...
                model.WhenValueChanged(x => x.DataType).Subscribe(x =>
                {
                    Redraw();
                }).DisposeWith(_compositeDisposable);
                
                model.WhenValueChanged(x => x.FixedPointShift).Subscribe(x =>
                {
                    Redraw();
                }).DisposeWith(_compositeDisposable);
            }
        ...
        public void DrawSignal(DrawingContext context, WaveModel model)
        {
            ...
            for (var i = 0d; i < Bounds.Width;)
            {
                ...
                var nextChangeTime = index < 0 ? model.Signal.GetChangeTimeFromIndex(0) : model.Signal.GetChangeTimeFromIndex(index + 1);
                ...
                else
                {
                    if (sWidth > 20)
                    {
                        ...
                        context.DrawText(
                            new FormattedText(cutText, CultureInfo.CurrentCulture, FlowDirection.LeftToRight, typeface, fontSize, Brushes.White),
