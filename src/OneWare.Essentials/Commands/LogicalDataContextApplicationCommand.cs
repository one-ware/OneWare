...
public class LogicalDataContextApplicationCommand<T>(string name, Action<T> action)
    : ApplicationCommandBase(name) where T : class, ILogical
{
    ...
    {
        return source is StyledElement { DataContext: T } ||
               source.FindLogicalAncestorWithDataContextType<T>() is not null;
    }
}