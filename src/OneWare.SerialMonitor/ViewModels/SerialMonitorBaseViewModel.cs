...
namespace OneWare.SerialMonitor.ViewModels
{
    public abstract class SerialMonitorBaseViewModel : OutputBaseViewModel
    {
        ...
        private string _selectedLineEncoding;
        ...
        public SerialMonitorBaseViewModel(ISettingsService settingsService, string iconKey) : base(iconKey)
        {
            ...
            _selectedBaudRate = settingsService.GetSettingValue<int>("SerialMonitor_SelectedBaudRate");
            settingsService.Bind("SerialMonitor_SelectedBaudRate", 
                this.WhenValueChanged(x => x.SelectedBaudRate));

            _selectedLineEncoding = settingsService.GetSettingValue<string>("SerialMonitor_SelectedLineEncoding");
            settingsService.Bind("SerialMonitor_SelectedLineEncoding", 
                this.WhenValueChanged(x => x.SelectedLineEncoding));
            
            _selectedLineEnding = settingsService.GetSettingValue<string>("SerialMonitor_SelectedLineEnding");
            settingsService.Bind("SerialMonitor_SelectedLineEnding", 
                this.WhenValueChanged(x => x.SelectedLineEnding));
        }
